...
async function sendEmail(to, firstName, lastName, company) {
  ...
  let mailOptions = {
    from: `Allan Xing <${process.env.EMAIL_USER}>`,
    to: to,
...
app.post("/send-email", async (req, res) => {
  const { firstName, lastName, company, companyDomain, customEmail } = req.body
  if (customEmail) {
    await sendEmail(customEmail, firstName, lastName, company)
  } else {
